...
function peticionServidorE(){
    ...
    if (CiudadOrigen != CiudadDestino) {
        ajax.open("GET","model/script5.php?" + obtenerQueryStringE());   
...
function respuestaServidorE() {
    if (ajax.readyState == 4 && ajax.status == 200) {
        ...
        operacionesE = JSON.parse(ajax.responseText);
        document.getElementById("Tarifa").innerHTML = operacionesE.tarifa;
        document.getElementById("Incremento").innerHTML = operacionesE.incremento;
        document.getElementById("ValorServicios").innerHTML = operacionesE.valorServicios;
        document.getElementById("Descuento").innerHTML = operacionesE.descuento;
        document.getElementById("CalculoTotal").innerHTML = operacionesE.calculoTotal;
    }
}
